...
Модули проекта:
- `базис гипотезы` — что предполагается о времени, гравитации и рефрактивном отклике;
- `фоновая космология` — отдельный вычислительный слой FRW;
- `observables / fitting` — отдельный data-facing слой;
- `код` — инструмент проверки, а не источник физического канона.
...
- `docs/00_overview.md`: карта гипотезы и модулей.
- `docs/01_interpretation.md`: физический базис, природа гравитации и времени.
- `docs/11_derivation_path.md`: канон `Θ_{μν}, Ψ -> θ_eff -> J_refr -> n -> ĝ_{μν}`.
- `docs/08_foundations.md`: минимальные математические требования, устойчивость, каузальность.
- `docs/02_background.md`: только фоновая космология и FRW-редукция.
- `docs/06_data_and_observables.md`, `docs/10_falsifiability.md`: только наблюдательные и fit-слои.

Принцип чтения:
- сначала `00 -> 01 -> 11 -> 08`;
- потом, если нужен фон, `02`;
- только после этого `06/10` и `fitting/`.
...
- Артефакты выполнения (`results/`, `tmp/`, кэш, LaTeX-intermediate) игнорируются через `.gitignore`.
- В корне оставлены только кодовые входные точки и ключевые каталоги.
- Старые `scripts/run_*.py` считаются внутренними шагами; рабочие точки входа: `scripts/entry_*.py`.
- Сравнение с внешними моделями относится только к observational-слою и не определяет физический базис гипотезы.
