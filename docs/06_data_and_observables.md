...
- empirical observables: прямые data-facing величины, которые идут в falsifiability;
...
- Атомный / химический предел: Θ–Ψ не должен менять стандартную релятивистскую химию; в этом режиме требуется n(θ_eff) -> 1, γ_PPN = 1, β_PPN = 1.
...
2) Преобразуйте форматы в простые CSV (без сетевого доступа):
   ...
   - strong-field события empirical-уровня (EHT ring catalog -> forward-модель на уровне событий):
     ...
     name,observable,target,sigma,mass_msun,distance_m,source. В empirical-контуре сейчас поддерживается только observable=ring_uas.
...
Примечание:
- echo-подобные величины и conservative compact-bound scans не входят в weak-field empirical-контур; для strong-field они остаются diagnostic-артефактами.
